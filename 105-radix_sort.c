...
/**
 * get_digit - Retrieves the digit at a given position in a number
...
/**
 * radix_pass - Performs a pass of radix sort based on a specific digit
...
void radix_sort(int *array, size_t size)
{
    ...
    for (i = 0; i < sd; i++)
    {
        ...
        old_array = new_array;
        new_array = temp_ptr;
        print_array(old_array, size); // Print the array after each pass
    }
    for (i = 0; i < size; i++)
        array[i] = old_array[i];
    free(ptr);
